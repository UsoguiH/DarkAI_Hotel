...
export default function Reception(props) {
	...
	useFrame((state) => {
		...
		if (phoneArrowRef.current) {
			phoneArrowRef.current.position.y = 1.2 + Math.sin(time * 2 + 1) * 0.05;
		}

		// Pulse the phone emissive glow
		if (phoneMeshRef.current) {
			const pulse = 0.3 + Math.sin(time * 3) * 0.2;
			phoneMeshRef.current.material.emissiveIntensity = pulse;
		}
		...
		if (phoneGroupRef.current) {
			...
			camera.getWorldDirection(direction);
			const angle = Math.atan2(direction.x, direction.z);
			phoneGroupRef.current.rotation.y = angle + Math.PI / 2;
		}
		if (phoneArrowRef.current) {
			const direction = new THREE.Vector3();
			camera.getWorldDirection(direction);
			const angle = Math.atan2(direction.x, direction.z);
			phoneArrowRef.current.rotation.y = angle + Math.PI / 2;
		}
		if (howItsMadeArrowRef.current) {
			const direction = new THREE.Vector3();
			camera.getWorldDirection(direction);
		...
		if (camera.position.z > 2.3 && camera.position.x > 8 && !isAnyPopupOpen) {
			...
			const howItsMadeIntersects = howItsMadeBoxRef.current
				...
				: [];
			const phoneIntersects = phoneBoxRef.current
				? raycaster.intersectObject(phoneBoxRef.current)
				: [];
			...
			if (phoneIntersects.length > 0) {
			...
			} else if (guestBookIntersects.length > 0) {
				...
				if (cursor === 'book' || cursor === 'help' || cursor === 'phone') {
					setCursor(null);
				}
	...
	return (
		...
		>
			...
			{/* Phone interaction box + visible phone mesh */}
			<mesh
				ref={phoneBoxRef}
				position={[-2, 1.05, 1.1]}
				scale={0.5}
			...
			{showPhone && (
				<group>
					...
					<Svg
						ref={phoneArrowRef}
						src={`data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="%23FF453A" d="M23.3711 0C25.0279 0 26.3711 1.34315 26.3711 3V84.9609L45.6289 104.217L23 126.843L0.371094 104.217L20.3711 84.2188V3C20.3711 1.34315 21.7142 0 23.3711 0ZM9.16309 104.217L23 118.052L36.8359 104.217L23 90.3818L9.16309 104.217ZM31.3418 104.076L22.8564 112.562L14.3711 104.076L22.8564 95.5908L31.3418 104.076Z"/></svg>`}
						position={[-2, 1.2, 1.04]}
						scale={0.002}
					/>
				</group>
